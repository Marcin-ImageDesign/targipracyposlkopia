...
var ChatHandler = new Class({
  ...
  ping : function() {
    ...
    var request = new Request.JSON({
      url : '/lite.php?module=chat&action=ping',
//      url : '/chat/ping',
...
//      url : this.options.basePath + 'application/lite.php?module=chat&action=join',
      url : '/lite.php?module=chat&action=join',
//      url : '/chat/join',
      data : {
        'format' : 'json',
        'room_id' : room_id
...
//      url : this.options.basePath + 'application/lite.php?module=chat&action=send',
      url : '/lite.php?module=chat&action=send',
//      url : '/chat/send',
      data : {
        'format' : 'json',
        'room_id' : room_id,
  ...
  status : function(state, type) {
    ...
    return (new Request.JSON({
      ...
      url : '/lite.php?module=chat&action=status',
//      url : '/chat/status',
      data : {
        'format' : 'json',
        'status' : state,
...
//      url : this.options.basePath + 'application/lite.php?module=chat&action=list',
      url : '/lite.php?module=chat&action=list',
//      url : '/chat/list',
      data : {
        'format' : 'json'
      },
...
//        'html' : ( Browser.Engine.gecko ? '<p><br /></p>' : '' ),
      ...
      // Submit
    this.elements.inputSubmit = new Element('input', {
      'id' : 'chat_input_submit',
      'class' : 'chat_input_submit',
      'type' : 'submit',
      'value' : translate_chat['label_chat_send']
    }).inject(this.elements.inputWrapper);
...
//        'type' : 'text'
      ...
      // Submit
    this.elements.inputSubmit = new Element('input', {
      'id' : 'chat_input_submit_' + identity,
      'class' : 'chat_input_submit',
      'type' : 'submit',
      'value' : translate_chat['label_chat_send']
    }).inject(this.elements.inputWrapper);
    ...
    this.elements.input.addEvents({
      'keypress' : function(event) {
        if( event.key == 'enter' ) {
          ...
          this.send();
          document.getElementById('chat_input').set('value', translate_chat['label_chat_message_send'] ).autoempty(translate_chat['label_chat_message_send']);
        }
      }.bind(this)
    });
    this.elements.inputSubmit.addEvents({
      'click' : function(event) {
          event.preventDefault();
          this.send();
          document.getElementById('chat_input').set('value', translate_chat['label_chat_message_send']).autoempty(translate_chat['label_chat_message_send']);
  ...
  send : function() {
    ...
    if( this.handler._supportsContentEditable() ) {
      ...
      if( Browser.Engine.webkit ) {
        ...
        this.elements.inputSubmit.destroy();
        delete this.elements.input;
        delete this.elements.inputSubmit;
        this.elements.input = new Element('textarea', {
          'id' : 'chat_input',
          'class' : 'placeholder'
//          'html' : ( Browser.Engine.gecko ? '<p><br /></p>' : '' )
    ...
    if( parseInt(data.state) >= 1 ) {
      if( !userEl ) {
        ...
        if( this.options.operator ){
            banLink = '<a href="#" class="putMessage" onClick="addSystemMessage(\'/kick ' + data.identity+'\');"><img src="/_images/icon/delete.png" style="margin-bottom: -3px;"></a> ';
        }
          
        var userHtml = '<span class="chat_user_name">'+banLink+'<a href="#">' + data.title + '</a></span>'; 
        if(data.operator){
            var userHtml = '<span class="chat_user_name chat_user_operator"><a href="#">' + data.title + '</a></span>'; 
        }  
        ...
        // Add system notice
        // fast disable info about user who join to room
...
//      })).inject(msgWrpr);
      ...
      var operatorClass = '';
      if(user.operator){
          operatorClass = 'author_is_operator';
      }

      (new Element('div', {
        'class' : 'chat_message_info',
        'html' : '\n\
  <span class="chat_message_info_author '+operatorClass+'"><a href="#">' + user.title + '</a></span>\n\
...
var ChatHandler_Utility = {
  ...
  imageSpec : '/_images/icon/tango_16/:name:.png',
...
function addSystemMessage(message){
    document.getElementById('chat_input').set('value', message).autoempty(translate_chat['label_chat_message_send']);
    return true;  
  }
